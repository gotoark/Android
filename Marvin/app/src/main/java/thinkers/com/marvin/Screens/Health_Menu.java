...
import thinkers.com.marvin.Screens.Adapters.MatchAdapter;
...
public class Health_Menu extends AppCompatActivity {
    MatchAdapter listAdapter;
    ...
    public void getMovies(){
        final ProgressDialog progressDialog = ProgressDialog.show(Health_Menu.this, "Loading...", "Please Wait", false, false);
        RestApiInterface apiInterface = RestApiClient.getClient()
                .create(RestApiInterface.class);

        Call<MatchResponse> movie = apiInterface.getMatches("0CWPTXYxJmaPWm0eJMGwsrlqR9K2");
        movie.enqueue(new Callback<MatchResponse>() {
            @Override
            public void onResponse(Call<MatchResponse> call, Response<MatchResponse> response) {
                Log.d("RESULT", "----------------------------------onResponse: " + response.code());
                Log.d("RESULT", "----------------------------------onResponse: " + call.request().headers());
                Log.d("RESULT", "----------------------------------onResponse: " + call.request().url());
                Log.d("RESULT", "----------------------------------onResponse: " + response.code());
                if (response.isSuccessful()) {
                    result = response.body().getMatches();
                    Log.d("RESULT", "----------------------------------onResponse: " + result.size());
                    final String[] issues_items = new String[result.size()];
                    for (int i = 0; i < result.size(); i++) {
                        //Storing names to string array
                        issues_items[i] = result.get(i).getTeam1();
              ...
              /*  ArrayAdapter adapter = new ArrayAdapter<String>(IssueList.this,R.layout.list_item_issue,items);
                ...
                issue_list.setAdapter(adapter);*/
                    ...
                    listAdapter = new MatchAdapter(result, Health_Menu.this, new MatchAdapter.PostItemListener() {
                        @Override
                        public void onPostClick(long id) {

                        }
                    });
                    RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(Health_Menu.this);
                    mRecyclerView.setLayoutManager(layoutManager);
                    mRecyclerView.setAdapter(listAdapter);
                    mRecyclerView.setHasFixedSize(true);
                    RecyclerView.ItemDecoration itemDecoration = new DividerItemDecoration(Health_Menu.this, DividerItemDecoration.VERTICAL);
    ...
    //Set Lisener
    public void setClikLiseners(final RecyclerView mRecyclerView, final Response<MatchResponse> response){
        ItemClickSupport.addTo(mRecyclerView).setOnItemClickListener(new ItemClickSupport.OnItemClickListener() {
            @Override
            public void onItemClicked(RecyclerView recyclerView, int i, View v) {
                // do it
                Intent intent=new Intent(Health_Menu.this,MoviesDetails.class);
                intent.putExtra("issue_id",result.get(i).getUniqueId());
                intent.putExtra("issue_title",result.get(i).getDate());
                intent.putExtra("issue_description",result.get(i).getSquad());
                intent.putExtra("issue_type",result.get(i).getTeam2());
                intent.putExtra("issue_level",result.get(i).getTeam1());
                intent.putExtra("issue_createdDate",result.get(i).getMatchStarted());
                intent.putExtra("issue_updated_date","No data Available");
                startActivity(intent);
            }
        });
